...
exports.doCompile = function(socket)
{
    ...
    var binaryFile = require('./staticVar.js').getBinaryFile();
    var gcc = spawn('gcc',[codeFile,'-g','-std=c99']);//着一句不能写到外面, 否则编译的是上次的结果
    //var gcc = spawn('gcc',[codeFile,'-g','-std=c99','-o','a.out']);//着一句不能写到外面, 否则编译的是上次的结果
    var str = "";
    var tmp = "";

    fs =require('fs');
    fs.exists(binaryFile,function(ex){
        if(ex)
        {

            console.log("文件存在");
            fs.unlink(binaryFile,function(e){
